...
const Rack = () => {
	...
	const handleShowReceipt = async (rutCliente, idReserva) => {
		...
		try {
			const res = await receiptService.findReceiptByRutAndReserva(
				rutCliente,
				idReserva
			);
						...
						{loadingDetails ? (
							...
							<div>
								...
								<p>
									...
									{new Date(reservationDetails.tiempoInicio).toLocaleString(
										"es-CL"
									)}
								</p>
								<p>
									<strong>Término:</strong>{" "}
									{new Date(reservationDetails.tiempoTermino).toLocaleString(
										"es-CL"
									)}
								</p>
								<p>
									<strong>Tipo:</strong>{" "}
									{tipoReservaLabel(reservationDetails.tipoReserva)}
								</p>
								<p>
									<strong>Cantidad de clientes:</strong>{" "}
									{reservationDetails.cantidadClientes}
								</p>
								<p>
									<strong>RUT Cliente principal:</strong>{" "}
									{reservationDetails.rutClientePrincipal}
								</p>
								<p>
								...
								>
									{reservationDetails.rutClientes.map((rut, idx) => (
										<li
											key={idx}
					...
					>
						...
						{loadingReceipt ? (
							...
							<div>
								...
								<p>
									<strong>RUT Cliente:</strong> {receiptDetails.rutCliente}
								</p>
								<p>
									<strong>Nombre Cliente:</strong>{" "}
									{receiptDetails.nombreCliente}
								</p>
								<p>
									<strong>Mail Cliente:</strong> {receiptDetails.mailCliente}
								</p>
								<p>
									<strong>Visitas Mensuales:</strong>{" "}
									{receiptDetails.visitasMensuales}
								</p>
								<p>
									<strong>¿Cumpleaños?:</strong>{" "}
									{receiptDetails.checkCumple ? "Sí" : "No"}
								</p>
								<p>
									<strong>Costo Inicial:</strong> $
									{receiptDetails.costoInicial?.toLocaleString("es-CL")}
								</p>
								<p>
									<strong>Descuento Grupo:</strong> $
									{receiptDetails.descuentoGrupo?.toLocaleString("es-CL")}
								</p>
								<p>
									<strong>Descuento Día Especial:</strong> $
									{receiptDetails.descuentoDiaEspecial?.toLocaleString("es-CL")}
								</p>
								<p>
									<strong>Descuento Frecuencia:</strong> $
									{receiptDetails.descuentoFrecuencia?.toLocaleString("es-CL")}
								</p>
								<p>
									<strong>Descuento Cumpleaños:</strong> $
									{receiptDetails.descuentoCumple?.toLocaleString("es-CL")}
								</p>
								<p>
									<strong>Costo Final:</strong> $
									{receiptDetails.costoFinal?.toLocaleString("es-CL")}
								</p>
								<p>
									<strong>Fecha Reserva:</strong>{" "}
									{receiptDetails.fechaReserva
										? new Date(receiptDetails.fechaReserva).toLocaleString(
												"es-CL"
										  )
										: ""}
								</p>
								<p>
									<strong>ID Reserva:</strong> {receiptDetails.idReserva}
								</p>
							</div>
						) : null}
					</div>
